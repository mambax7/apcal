...
class phpthumb_filters
{
    ...
    public function HistogramStretch(&$gdimg, $band = '*', $method = 0, $threshold = 0.1)
    {
        ...
        $keys = ['r' => 'red', 'g' => 'green', 'b' => 'blue', 'a' => 'alpha', '*' => (0 == $method) ? 'all' : 'gray'];
        $band = $band[0];
        if (!isset($keys[$band])) {
            return false;
        }
     ...
     * @param $gdimg_source
     * @param $rotate_angle
     * @param $config_background_hexcolor
     * @param $bg
    ...
    public function WatermarkText(&$gdimg, $text, $size, $alignment, $hex_color = '000000', $ttffont = '', $opacity = 100, $margin = 5, $angle = 0, $bg_color = false, $bg_opacity = 0, $fillextend = '', $lineheight = 1.0)
    {
        ...
        if (@is_readable($ttffont) && is_file($ttffont)) {
            ...
            if ('*' === $alignment) {
                ...
                $x_TL = false !== mb_stripos($fillextend, 'x') ? 0 : min($x1, $x2);
                $y_TL = false !== mb_stripos($fillextend, 'y') ? 0 : min($y1, $y2);
                $x_BR = false !== mb_stripos($fillextend, 'x') ? imagesx($gdimg) : max($x1, $x2);
                $y_BR = false !== mb_stripos($fillextend, 'y') ? imagesy($gdimg) : max($y1, $y2);
